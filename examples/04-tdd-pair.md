...
```bash
npx @oxog/imece init --desc "TDD Pair Programming Demo"

npx @oxog/imece register ali "test-driver" --caps "tdd,testing,typescript" --lead
npx @oxog/imece register zeynep "implementer" --caps "typescript,algorithms,refactoring"
```
...
npx @oxog/imece lock ali src/calculator.test.ts
...
# Ali unlocks
npx @oxog/imece unlock ali src/calculator.test.ts

# Notifies Zeynep
npx @oxog/imece send ali zeynep "TDD Round 1: Addition" \
...
npx @oxog/imece lock zeynep src/calculator.ts
...
# Unlocks
npx @oxog/imece unlock zeynep src/calculator.ts

# Notifies Ali
npx @oxog/imece send zeynep ali "Round 1: Green! ✅" \
...
npx @oxog/imece send ali zeynep "TDD Round 2: Edge Cases" \
...
```bash
npx @oxog/imece lock zeynep src/calculator.test.ts
# Runs tests
# All pass! (implementation already handles these)
npx @oxog/imece unlock zeynep src/calculator.test.ts

npx @oxog/imece send zeynep ali "Round 2: Already Green! ✅" \
...
npx @oxog/imece send ali zeynep "TDD Round 3: Subtraction" \
...
npx @oxog/imece send zeynep ali "Round 4: Green! ✅" \
...
```bash
npx @oxog/imece send ali zeynep "Refactor: Extract Operation Type" \
...
npx @oxog/imece lock ali src/calculator.ts
# Refactors to use Operation type
npx @oxog/imece lock ali src/calculator.test.ts
# Updates tests to use shared type
npx @oxog/imece unlock ali src/calculator.test.ts
npx @oxog/imece unlock ali src/calculator.ts

npx @oxog/imece send ali zeynep "Refactor Complete" \
...
npx @oxog/imece send zeynep ali "Refactor LGTM! ✅" \
...
npx @oxog/imece heartbeat ali
npx @oxog/imece heartbeat zeynep

npx @oxog/imece broadcast ali "Starting TDD pair session with Zeynep"
npx @oxog/imece broadcast zeynep "Ready to pair with Ali on calculator feature"
```
...
npx @oxog/imece task create ali zeynep "TDD: Calculator Module" \
...
npx @oxog/imece task note <task-id> zeynep "Addition feature complete - 3 test cycles"
npx @oxog/imece task note <task-id> zeynep "Subtraction complete - 2 test cycles"
```
...
```bash
# Mark task progress
npx @oxog/imece task note <task-id> zeynep "Session complete: 4 operations implemented, 1 refactor"

# Both go idle
npx @oxog/imece heartbeat ali  # Sets to online/idle
npx @oxog/imece heartbeat zeynep

# Broadcast completion
npx @oxog/imece broadcast ali "TDD session complete. Calculator module 80% done."
```
...
npx @oxog/imece send zeynep ali "Role swap" \
...
npx @oxog/imece send zeynep ali "Alternative implementation" \
  --body "Instead of separate functions, what about a Calculator class?\n\nPros:\n- Can track history\n- Chain operations\n- More extensible\n\nCons:\n- More complex\n- Breaking change\n\nThoughts?" \
  --type question

# Ali responds
npx @oxog/imece reply ali <msg-id> "Good idea! Let's go with the class approach. I'll write the test structure."
```
...
npx @oxog/imece send ali zeynep "TDD: Async Round" \
...
npx @oxog/imece send zeynep ali "Multiply: Green! ✅" \
...
```bash
# See all TDD cycles
npx @oxog/imece timeline | grep -E "(test|implement|refactor)"

# Check test coverage discussions
npx @oxog/imece search "test"

# See who did what
npx @oxog/imece timeline --agent ali
npx @oxog/imece timeline --agent zeynep
```
